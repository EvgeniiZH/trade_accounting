...
export default function Layout({ children }: LayoutProps) {
  ...
  const getPageTitle = () => {
    // Основные пункты меню по точному пути
    const directMatch = menuItems.find((item) => item.path === location.pathname);
    if (directMatch) return directMatch.text;

    // Страница редактирования расчёта: /calculations/:id/edit
    if (location.pathname.startsWith('/calculations/') && location.pathname.endsWith('/edit')) {
      return 'Редактирование расчёта';
    }
